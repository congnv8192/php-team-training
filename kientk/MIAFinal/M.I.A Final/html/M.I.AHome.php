...
<title>M.I.A Home of Ebooks</title>
...
// actions if a category is selected or not:
if (!isset($_GET['cat'])){
//select all book
$result=mysql_query("SELECT id,title,price,img_link,author,read_online_link FROM book");
}

else {
//select book which fit the category
$query=("SELECT id,title,price,img_link,author,read_online_link,category_name FROM book,`fit_category(r)`,category WHERE `fit_category(r)`.book_id=book.id AND `fit_category(r)`.category_alias = category.category_alias AND `fit_category(r)`.category_alias = ".$_GET['cat']);
...
//actions if there is no book which fit the selected category

if (mysql_fetch_array($result)=='') 
	echo '<div clsss = "item"><h5 style="font-size:15px">This category currently has no book. We are very sory for this inconvenience. Please consider uploading a book to our site.</h5></div>';
